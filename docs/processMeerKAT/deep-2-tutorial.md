...
This way we reset the list of scripts in our config file, and set `verbose=False` and `submit=False`. We will manually remove the scripts we already ran in [step 23](#23-edit-submit_pipelinesh), so leave the `scripts` parameter as is.
